...
import {Box, Icon, Menu, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow} from "@mui/material";
...
import {
  ...
  theaterIncomeTmch,
} from "../../../../Redux-Slice/incomeCollectionTmchSlice/incomeProcedureTmchSlice";
import {
  getPhaSalePartTmch1,
  getPhaSalePartTmch2,
  getPhaSalePartTmch3,
...
const IncomeReports = () => {
  ...
  // Ensure number checking function
  ...
  }, [general, otherType, misCollection]);
  ...
  }, [misCollection, collAgainSale, tax, netAmount, discount, GrosPharma, rndOff, pharmaAmount, grand]);

  const totalCounterCollection = useMemo(
    () =>
      collAgainSale +
      ensureNumber(misCollection?.[0].advanceCollection || 0) +
  ...
  const onClickFuncLevelOne = useCallback(
    async (data) => {
      ...
      const endpoints = {
        BED: "/incomeDetlTmch/bedIncome",
        ROOM: "/incomeDetlTmch/roomRentIncome",
        NS: "/incomeDetlTmch/nsIncome",
        OTHERS: "/incomeDetlTmch/otherIncome",
        CONSULTING: "/incomeDetlTmch/consultingIncome",
        THEATRE: "/incomeDetlTmch/theaterIncome",
        OPERATION: "/incomeDetlTmch/surgeonIncome",
        ANESTHESIA: "/incomeDetlTmch/anesthetiaIncome",
        CARDIOLOGY: "/incomeDetlTmch/cardiologyIncome",
        DISPOSIBLE: "/incomeDetlTmch/disPosibleItemIncome",
        ICU: "/incomeDetlTmch/icuIncome",
        "ICU PROC": "/incomeDetlTmch/icuprocedureIncome",
        RADIOLOGY: "/incomeDetlTmch/radiologyIncome",
        LAB: "/incomeDetlTmch/laboratoryIncome",
        MRI: "/incomeDetlTmch/mriIncome",
        DIET: "/incomeDetlTmch/dietIncome",
      };
  ...
  const getPharmacyDetl = useCallback(async () => {
    ...
    try {
      const [res1, res2, res3, res4] = await Promise.all([
        axiosinstance.post("/incomeDetlTmch/pharmacyIncomePart1", state),
        axiosinstance.post("/incomeDetlTmch/pharmacyIncomePart2", state),
        axiosinstance.post("/incomeDetlTmch/pharmacyIncomePart3", state),
        axiosinstance.post("/incomeDetlTmch/pharmacyIncomePart4", state),
      ]);
  ...
  const onClickCreditInsuranceBill = useCallback(async () => {
    ...
    try {
      ...
      console.log("Error fetching credit insurance details:", error);
    }
  }, [state]);
  ...
  return (
    <Box flex={1} sx={{backgroundColor: "lightgray", p: "1%"}}>
      <MenuButton navigateTo={"hospital_income_tmch"} />
      <ReportModal layout={layout} setLayout={setLayout} state={state} data={modalData} name="TRAVANCORE MEDICAL COLLEGE & HOSPITAL" />
      <PharmacyReoprtModal layout={layout1} setLayout={setLayout1} state={state} data={pharmacyDetl} name="TRAVANCORE MEDICAL COLLEGE & HOSPITAL" />
      <CreditInsuranceBillModal layout={layout2} setLayout={setLayout2} state={state} data={credtInsuranceDetl} name="TRAVANCORE MEDICAL COLLEGE & HOSPITAL" />
      <UnsettledAmntModal layout={layout3} setLayout={setLayout3} state={state} data={unsettled} name="TRAVANCORE MEDICAL COLLEGE & HOSPITAL" />
      <AdvanceCollcetionDetl layout={layout4} setLayout={setLayout4} state={state} data={advanceCollDetl} name="TRAVANCORE MEDICAL COLLEGE & HOSPITAL" />
      {/* CREDIT INSURANCE BILL COLLECTION  */}
      <CreditInsurBillCollModal layout={layout5} setLayout={setLayout5} state={state} data={credInsuranceCol} name="TRAVANCORE MEDICAL COLLEGE & HOSPITAL" />
