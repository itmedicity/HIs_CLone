...
const CreditInsuranceBillModal = ({layout, setLayout, state, data, name}) => {
  const conCatArray = useMemo(() => {
    const arrays = [data.credit1, data.credit2, data.credit3, data.credit4, data.credit5, data.credit6].filter((arr) => Array.isArray(arr)); // Ensure only arrays are concatenated
    return [].concat(...arrays);
  }, [data.credit1, data.credit2, data.credit3, data.credit4, data.credit5, data.credit6]);

  // Memoize total amount calculation
  const totalAmountTwo = useMemo(() => {
    return conCatArray.reduce((acc, curr) => acc + (curr.AMT || 0), 0);
  }, [conCatArray]);

  return (
    <Modal open={!!layout} onClose={() => setLayout(undefined)}>
      <ModalDialog aria-labelledby="layout-modal-title" aria-describedby="layout-modal-description" layout={layout} sx={{backgroundColor: "lightgrey", overflow: "auto"}}>
        <Box>
              ...
              <Table padding="none" sx={{}} size="small" aria-label="a dense table">
                ...
                >
                  ...
                  >
                    ...
                    <TableCell padding="none" variant="body" size="small" align="left" sx={{width: "15%", fontWeight: "bolder", fontSize: "12px", pl: 2}}>
                      Patient Name
                    </TableCell>
                    <TableCell padding="none" variant="body" size="small" align="left" sx={{width: "10%", fontWeight: "bolder", fontSize: "12px"}}>
                      Pt No
                    </TableCell>
                    <TableCell padding="none" variant="body" size="small" align="left" sx={{width: "15%", fontWeight: "bolder", fontSize: "12px", pr: 2}}>
                      Bill No
                    </TableCell>
                ...
                <TableBody>
                  ...
                  >
                    ...
                    <TableCell align="left" colSpan={7} sx={{fontWeight: "bolder", fontSize: "12px", textTransform: "capitalize"}}>
                      Billing
                    </TableCell>
                  </TableRow>
                  {conCatArray?.length > 0 ? (
                    conCatArray?.map((e, idx) => (
                      <TableRow key={idx}>
                    ...
                    <TableRow>
                      <TableCell colSpan={8} align="center" sx={{fontSize: "12px", py: 2}}>
                        No data available
                      </TableCell>
                    </TableRow>
                  )}
                ...
                <TableFooter>
                  <TableRow>
                    ...
                    <TableCell align="left" sx={{width: "15%", fontSize: "12px", textTransform: "capitalize", fontWeight: 700, color: "black"}}>
                      Total
                    </TableCell>
                    ...
                    <TableCell align="right" sx={{width: "15%", fontSize: "12px", pr: 2, fontWeight: 700, color: "black"}}></TableCell>
                    <TableCell align="right" sx={{width: "15%", fontSize: "12px", pr: 2, fontWeight: 700, color: "black"}}>
                      {totalAmountTwo?.toLocaleString("en-US", {minimumFractionDigits: 2})}
                    </TableCell>
                    <TableCell align="right" sx={{width: "15%", fontSize: "12px", pr: 2, fontWeight: 700, color: "black"}}></TableCell>
                    <TableCell align="right" sx={{width: "15%", fontSize: "12px", pr: 2, fontWeight: 700, color: "black"}}></TableCell>
                  </TableRow>
...
export default memo(CreditInsuranceBillModal);
