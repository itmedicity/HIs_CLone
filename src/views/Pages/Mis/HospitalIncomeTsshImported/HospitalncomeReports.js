...
const HospitalncomeReports = () => {
  /**
   *  Hospital income Statement - imported TSSH
   */
  ...
  const handleClick = useCallback(async () => {
    ...
    try {
      ...
      //GET THE TMCH DATA
      const result = await axiosinstance.post("/admission/getTsshIncome", postDataForMysql);
      const {success, data} = result.data;
      if (success === 1) {
        navigate("/Menu/income-reports-imTssh", {
          state: {
