...
const IncomeReports = () => {
  ...
  const advanceCollection = (misCollection?.[0].advanceCollection || 0).toLocaleString("en-US", {
  ...
  const creditInsuranceBillCollection = (misCollection?.[6].creditInsuranceBillCollection || 0).toLocaleString("en-US", {
    minimumFractionDigits: 2,
  });
  const ipConsolidatedDiscount = (misCollection?.[7].ipConsolidatedDiscount || 0).toLocaleString("en-US", {
    minimumFractionDigits: 2,
  });
  const ipPreviousDayDiscount = (misCollection?.[8].ipPreviousDayDiscount || 0).toLocaleString("en-US", {
    minimumFractionDigits: 2,
  });
  const unsettledAmount = (misCollection?.[9].unsettledAmount || 0)?.toLocaleString("en-US", {minimumFractionDigits: 2});
  const ippreviousDayCollection = (misCollection?.[10].ippreviousDayCollection || 0).toLocaleString("en-US", {
    minimumFractionDigits: 2,
  });
  const creditInsuranceBill = (misCollection?.[11].creditInsuranceBill || 0).toLocaleString("en-US", {
    minimumFractionDigits: 2,
  });
  ...
  }, [state, collection, misGroup, proIncome, pharmacyIncome, misGroupList, misReortList, dispatch]);
  ...
  const totalCounterCollection = useMemo(
    () =>
      ...
      ensureNumber(misCollection[6]?.creditInsuranceBillCollection ?? 0) +
      ensureNumber(misCollection[10]?.ippreviousDayCollection ?? 0) -
      ensureNumber(misCollection[1]?.advanceRefund ?? 0),
    [collAgainSale, misCollection[0]?.advanceCollection, misCollection[6]?.creditInsuranceBillCollection, misCollection[10]?.ippreviousDayCollection, misCollection[1]?.advanceRefund]
  );
  ...
  const {ipConatedDiscount, advSettled, creditInsurBill, unsettledAmnt, groupCollection, groupTax, groupNet, groupDis, groupGross, roundOff, groupNetddctRoundoff} = useMemo(() => {
    ...
    misCollection?.[7].ipConsolidatedDiscount,
    misCollection?.[2].advanceSettled,
    misCollection?.[11].creditInsuranceBill,
    misCollection?.[9].unsettledAmount,
    grand?.groupTax,
    grand?.groupNet,
    grand?.groupDis,
    grand?.groupGross,
